...
const Sprint1 = () => {
  ...
  // ✅ Sprint backlog loaded from Mongo (items that have sprintName=currentSprint.name)
  const [sprintBacklog, setSprintBacklog] = useState([]);

  // Editing
  const [isEditing, setIsEditing] = useState(false);
  const [originalSprintName, setOriginalSprintName] = useState(""); // track if editing existing sprint
  ...
  // Regex for time format like "2w 4d 6h 45m"
  ...
  // ----------------------------------------
  // ✅ Load sprint backlog items from Mongo
  // ----------------------------------------
  const loadSprintItems = async (sprintName) => {
    if (!sprintName) {
      setSprintBacklog([]);
      return;
    }
    ...
    try {
      ...
      const normalized = (res.data || []).map((it) => ({
        ...
        // optional fields (make sure backend schema allows them)
  ...
  // whenever current sprint changes, refresh its items
  useEffect(() => {
    loadSprintItems(currentSprint.name);
  }, [currentSprint.name]);

  // ---------------------------
  // Sprint form change handler
  // ---------------------------
  ...
  const handleSaveSprint = async () => {
    ...
    if (!allowedSprintNames.includes(currentSprint.name)) {
      alert("Invalid sprint name. Choose Sprint 1/2/3.");
      return;
    }
    ...
    try {
      if (!isEditing) {
        // prevent duplicate
        const nameUsed = sprints.some((s) => s.name === currentSprint.name);
        if (nameUsed) {
          alert("This sprint already exists. Edit it instead.");
          return;
        }

        await api.post("/api/sprints", currentSprint);
      } else {
        // update
        const targetName = originalSprintName || currentSprint.name;
        await api.put(`/api/sprints/${encodeURIComponent(targetName)}`, currentSprint);
      }

      await loadSprints();

      setIsEditing(false);
      setOriginalSprintName("");
      setCurrentSprint({
        name: "",
        startDate: "",
        endDate: "",
      ...
      alert("✅ Sprint saved to MongoDB.");
    } catch (err) {
      console.error("❌ Failed to save sprint:", err);
      alert("Failed to save sprint (server error).");
    }
  };

  // ---------------------------
  // ✅ Edit sprint (load into form)
  // ---------------------------
  const handleEditSprint = (sprintName) => {
    const sprintToEdit = sprints.find((s) => s.name === sprintName);
    ...
    setCurrentSprint({
      ...
      progress: sprintToEdit.progress || "Not Started",
    });

    setIsEditing(true);
    setOriginalSprintName(sprintToEdit.name);
  };
  ...
  const handleDeleteSprint = async (sprintName) => {
    ...
    try {
      ...
      alert("Failed to delete sprint (server error).");
    }
  };

  // ---------------------------
  // ✅ Update a sprint backlog item (Mongo)
  // ---------------------------
  const updateBacklogItem = async (clientId, patch) => {
    try {
      await api.put(`/api/backlog/${encodeURIComponent(clientId)}`, patch);

      // Refresh list (simplest & consistent)
      await loadSprintItems(currentSprint.name);
    } catch (err) {
      console.error("❌ Failed to update item:", err);
      alert("Failed to update item (server error).");
    }
  };

  // Status change handler
  const handleStatusChange = async (id, newStatus) => {
    // If marking completed, ask completion date
    if (newStatus === "Completed") {
      ...
      if (Number.isNaN(dateObj.getTime())) {
        setCompletionDateError("Invalid date format.");
  ...
  const handleMoveBackToProductBacklog = async (item) => {
    const ok = window.confirm("Move this task back to Product Backlog?");
    if (!ok) return;
  ...
  const handleAddToSprintBacklog = async (item, sprintName) => {
    ...
    // persist: move PB item into sprint by setting sprintName
    await updateBacklogItem(item.id, { sprintName });
  };
  ...
  const sortBacklog = (list) => {
    return [...list].sort((a, b) => {
      ...
      if (backlogSortCriteria === "priority") {
        return backlogSortOrder === "asc"
          ? a.priority.localeCompare(b.priority)
          : b.priority.localeCompare(a.priority);
      }
      if (backlogSortCriteria === "status") {
        return backlogSortOrder === "asc"
  ...
  const sortedSprints = useMemo(() => sortSprints(sprints), [sprints, sprintSortCriteria, sprintSortOrder]);
  const sortedSprintBacklog = useMemo(() => sortBacklog(sprintBacklog), [sprintBacklog, backlogSortCriteria, backlogSortOrder, backlogSortDeveloperOrder]);
